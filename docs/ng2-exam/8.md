...
我们有七章，你应该感觉很好。到目前为止，您所学到的是我们在最后几章中构建的应用的直接结果。我们认为，您现在应该对该框架、其工作方式以及支持的内容有充分的了解。有了这些知识，一旦我们开始构建一些规模合适的应用，就会出现一些常见的问题/模式，例如：

*   我们如何认证用户并控制其访问（授权）？
*   我们如何确保应用的性能足够好？
*   我的应用需要本地化内容。我该怎么办？
*   我可以使用哪些工具加快应用开发？
*   我有一个 AngularJS 应用。如何迁移它？
...
*   **验证 Angular 应用**：这是一个常见要求。我们将研究如何在 Angular 中支持基于 cookie 和令牌的身份验证。
*   **Angular 性能**：必须有一个常规的性能部分，因为我们试图详细说明是什么使 Angular 性能更好，以及您可以做些什么来加快应用的速度。
*   **将 AngularJS 应用迁移到 AngularJS**：AngularJS 和 Angular 是完全不同的野兽。在本章中，您将学习如何逐步将 AngularJS 应用迁移到 AngularJS。
...
想象一下这里的一个场景：我们正在构建一个新的应用，鉴于 Angular 框架的强大功能，我们一致决定使用 Angular。伟大的接下来呢？接下来是建立项目的平凡过程。

虽然这是一项平凡的活动，但它仍然是任何参与的关键部分。建立一个新项目通常包括：

*   创建标准文件夹结构。这有时会受到服务器框架的影响（例如*RoR*、*ASP.Net*、*Node.js*等）。
*   将标准资源添加到特定文件夹。
*   在我们开发基于 Angular 的 web 应用的情况下，设置构建包括：
...
*   **角式起动器**[http://bit.ly/ng-starter](http://bit.ly/ng-starter) ：这个种子回购协议为任何希望快速启动并运行 Angular 和 TypeScript 的人提供了一个 Angular 启动程序。它使用 Webpack（模块绑定器）来构建我们的文件并帮助制作样板文件。这是一个包含大量集成的完整构建系统。
...
**摇树**对一个过程来说可能是一个古怪的名字，但它确实做到了它所说的！当我们使用 TypeScript（或 ES2015）构建包含*模块*和*导出*的应用时，一个模块绑定器，如*汇总*（[http://rollupjs.org](http://rollupjs.org) 可以对这些代码进行静态代码分析，确定哪些代码部分从未使用过，并在捆绑释放位之前移除它们。当添加到应用的构建过程中时，此类模块绑定器可以分析框架位、任何第三方库和应用代码，以在创建绑定之前删除任何死代码。*树抖动会导致巨大的尺寸缩减，因为您不捆绑不使用的框架位。*
...
使用 AoT 编译，视图模板（HTML）在服务器端预先编译。此编译再次作为应用构建过程的一部分完成，在该过程中，Angular 编译器（节点包）的服务器版本编译应用中的每个视图。

编译完所有模板后，根本不需要将 Angular 编译器位发送到客户端。树抖动现在可以摆脱编译器，创建一个更精简的框架包。Angular CLI 支持 AoT 编译，可用于生产构建。
...
任何具有成熟 JavaScript 框架的 web 应用的初始加载时间通常都很慢。这种影响在移动设备上更为明显，在移动设备上，JavaScript 引擎可能没有桌面客户端强大。为了获得更好的用户体验，必须优化框架的初始加载时间，特别是对于移动设备。

在初始加载时间和重新渲染视图时，**Angular2 比 AngularJS**快五倍。随着 Angular 团队开发框架，这些数字越来越好。

此外，AoT 编译也可以提高应用的初始加载时间，因为不需要耗时的活动（视图编译）。
...
对于 Angular，最大的架构变化是一个单独的渲染层。现在，Angular 应用有两层：

*   **应用层**：这是我们的代码所在的层。它使用渲染器层上的抽象构建与之交互。我们在*深入 Angular 指令*中看到的渲染器类是我们用来与渲染层交互的接口。
...
服务器端的**预渲染**是另一种提高 Angular 应用初始加载时间的技术。这项技术在移动设备上非常有用，因为它大大提高了感知负载时间。

服务器端呈现在客户端呈现开始之前负责初始页面加载（此后处理视图呈现）。

在这种情况下，当用户请求一个视图/页面时，服务器上的一个软件会生成一个完全物化的 HTML 页面，其中包含预绑定到视图的数据，并将其与一个小脚本一起发送到客户端。因此，立即呈现应用视图，为交互做好准备。当框架在后台加载时，第一次发送的小脚本将捕获所有用户输入并使其可用于框架，从而允许框架在加载后重播交互。
...
在当今世界，多核 CPU 是标准，但 JavaScript 执行仍然是单线程的。我们需要一种标准/机制来为我们的应用利用这些空闲内核。网络工作者完全符合这一要求，因为大多数现代浏览器都支持他们，所以我们都应该编写利用网络工作者的代码。
...
Angular 试图通过将 web worker 的使用集成到框架本身来缓解这些限制。它通过在 web worker 线程中运行完整的应用（渲染部分除外）来实现这一点。

该框架负责 web worker 内部运行的应用代码与主 UI 线程内部运行的呈现程序之间的通信。从开发人员的 Angular 来看，没有明显的差异。

由于渲染器层在 Angular 方向上的分离，这再次成为可能。下图显示了在应用主线程上运行的层以及在 web worker 中运行的层：
...
Angular 的渲染抽象再次打开了许多集成途径，特别是在移动平台上。与在移动浏览器上运行应用不同，可以创建 Angular 渲染器，以利用设备的本机 UI 功能。

该领域的两个著名项目是平台渲染器：

*   **反应物**[http://bit.ly/rnative](http://bit.ly/rnative) ：ReactNative（[的渲染器 http://bit.ly/ng-rnative](http://bit.ly/ng-rnative) ）。它允许使用 ReactNative 的渲染功能编写 Angular 应用。
...
*ReactNative*和*NativeScript*等应用平台在为本地移动平台（iOS 和 Android）提供基于 JavaScript 的 API 方面已经做得非常出色，允许我们使用熟悉语言的单一代码库。Angular 渲染器更进一步。通过 Angular 集成，可以在浏览器和移动设备之间共享大量代码。只在视图模板和视图相关服务（如对话框、弹出窗口等）方面有所不同。
...
AngularJS 上 Angular 的主要性能改进之一是*变化检测*在 AngularJS 中的工作方式。开箱即用的 Angular 变化检测速度非常快，可以进一步调整以获得更好的结果。

接下来的几节将深入讨论 Angular 变化检测。在大规模建造任何东西时，这是一个需要理解的重要话题。它还可以帮助我们调试一些场景，在这些场景中，更改检测似乎没有像广告中所宣传的那样工作。
...
Angular 的*数据绑定引擎*在将视图与模型数据（组件数据）绑定方面做得很好。这些是实时绑定，Angular 在其中使视图与模型更改保持同步。每当模型更改时，绑定引擎都会重新渲染依赖于模型的部分视图。要管理此视图模型同步，Angular 需要知道模型何时更改以及更改的确切内容。这就是**变化检测**的意义所在。在应用执行期间，Angular 经常执行我们称之为**的更改检测运行**来确定更改的内容。
...
在接下来的几节中，我们将探讨 Angular 变化检测基础设施如何工作，何时触发变化检测，以及我们如何影响 Angular 变化中的变化检测行为。



# 更改检测设置

这一切都始于为视图上渲染的每个组件设置 Angular 变化检测器。由于每个 Angular 应用都是组件的层次结构，因此这些变化检测器也设置在同一层次结构中。下图突出显示了*训练生成器*应用在某个时间点的**变更检测器层次结构**：
...
Angular 做得更好，因为在应用执行过程中，更改检测只在特定时间执行。仔细考虑任何 web 应用；什么导致视图更新？
...
出于显而易见的原因，Angular 变化检测也只有在出现上述任何一种情况时才起作用。
...
Angular 变化检测使用相同的技术在区域内执行代码。此代码可以是*事件处理程序*，在完成前内部进行更多的同步异步调用，也可以是`setTimeout`/`setInterval`操作，可能再次需要 UI 更新。

Angular 变化检测框架订阅执行区域的`onMicrotaskEmpty`可观测值，并在转弯完成时启动变化检测。下图突出显示了在单击按钮时运行类似于刚才描述的代码时发生的情况：
...
`ApplicationRef`类跟踪整个应用中连接的所有更改检测器，并在应用级区域对象触发`onMicrotaskEmpty`事件时触发更改检测循环。我们将很快讨论在这个变化检测过程中会发生什么。
...
一旦触发任何异步事件回调（事件处理程序的执行也是一个异步活动），Angular 将首先执行附加到回调的应用代码。此代码执行可能会导致一些模型更新。执行回调后，Angular 需要通过触发*更改检测运行*来响应更改。
...
1.  从 7 分钟训练应用中打开`start.component.html`并将最后一个`div`更新为：
...
3.  运行应用，加载起始页，然后查看浏览器控制台。Angular 记录了许多错误，如下所示：
...
尽管进行了所有这些优化，但仍可能存在遍历完整组件树的性能不够的情况。当我们必须在视图上呈现一个大数据集，保持绑定完整时，这一点尤其正确。好消息是可以调整 Angular 变化检测机制。
...
如果我们可以帮助 Angular 确定应用状态的哪些部分被更新，Angular 就可以非常聪明地知道它通过组件树的哪些部分来检测更改。我们通过将应用数据存储在一些特殊的数据结构中来实现这一点，这些数据结构有助于 Angular 确定需要检查哪些组件以进行更改。

有三种方法可以使 Angular 变化检测更智能。
...
让我们通过一个例子来了解不可变数据结构在 Angular 上下文中的帮助。
...
使用不变的数据结构开发应用与标准开发范式不同，在标准开发范式中，应用状态是完全可变的。我们在本节中强调的是 Angular 如何利用不可变的数据结构来优化更改检测过程。

*观测值*是另一种数据结构，可以帮助我们优化 Angular 变化检测。



# 使用可见光

**可观测项**是当内部状态发生变化时触发事件的数据结构。Angular*事件基础设施*广泛使用*可观察物*将组件的内部状态传达给外部世界。

虽然我们使用了可观察的输出属性（`EventEmitter`类）来引发事件，*输入**属性*也可以获取可观察的。这种可观察的输入有助于优化 Angular 变化检测。
...
以一个公共聊天室应用为例，它正在接收来自数千名连接到它的人的消息。如果我们不断地提取消息并刷新 DOM，应用可能会变得无响应。在这种情况下，我们可以在聊天应用组件树的某些部分禁用更改检测，并手动触发更改检测，以在特定的时间间隔更新 UI。
...
接下来，我们将看一看大多数应用都有的另一个常见要求：对用户进行身份验证。



# 处理身份验证和授权

大多数（如果不是全部的话）应用都需要对其用户进行身份验证/授权。我们可能会说，身份验证和授权更像是服务器的问题，而不是客户端的问题，这是正确的。不过，客户端需要适应并集成服务器强加的身份验证和授权要求。

在典型的应用执行工作流中，应用首先加载一些局部视图，然后调用从服务器提取数据，最后将数据绑定到其视图。显然，*视图*和*远程数据 API*是需要保护的两项资产。

为了保护这些资源，我们需要了解服务器上典型应用的安全性。保护任何 web 应用的安全主要有两种方法：*基于 cookie 的身份验证*和*基于令牌的身份验证*。它们中的每一个都需要在客户端进行不同的处理。接下来的两部分描述了我们如何与这两种方法进行集成。



# 基于 Cookie 的身份验证

如果服务器堆栈支持此身份验证机制，则此身份验证机制最容易实现。它是非侵入性的，可能需要对 Angular 应用进行最小程度的更改。**基于 Cookie 的身份验证**包括设置浏览器 Cookie 以跟踪用户身份验证会话。以下序列图解释了典型的基于 cookie 的身份验证工作流：
...
正如我们所看到的，使用这种方法，不涉及 Angular 基础设施，或者只涉及最小的 Angular 基础设施。甚至登录页面也可以是标准的 HTML 页面，它只向登录端点发送数据以进行身份验证。如果用户登录 Angular 应用，则意味着该用户已通过身份验证。
...
在基于 cookie 的应用身份验证中，如果应用想要获取用户上下文，则会公开一个服务器端点（例如【T0）】，该端点返回登录用户的特定数据。然后，客户端应用可以实现一个服务，例如加载和缓存用户配置文件数据的`UserService`。

这里描述的场景假设 API 服务器（返回数据的服务器）和承载应用的站点位于单个域中。情况可能并非总是如此。即使对于*私人教练*来说，数据也驻留在*蒙古 AB*服务器上，而应用驻留在不同的服务器上（即使是本地的）。我们已经知道，这是一种跨域访问，它也有自己的一系列挑战。

在这种设置中，即使 API 服务器能够对请求进行身份验证并将 cookie 发送回客户端，客户端应用也不会在后续请求中发送身份验证 cookie。
...
在典型的基于令牌的身份验证设置中，视图是公开的，但 API 是安全的。如果应用试图通过 API 调用提取数据，而没有将适当的令牌附加到传出请求，服务器将返回一个*HTTP 401 Unauthorized*状态代码。

在 Angular 中，启用令牌身份验证是一项很好的工作。它涉及设置登录页面/组件、保护条件、身份验证服务和跨应用共享的身份验证上下文。如果您开始这一旅程，请确保寻找 Angular 库/模块，使集成更容易。`ngx-auth`图书馆（[http://bit.ly/ngx-auth](http://bit.ly/ngx-auth) 可以是一个很好的起点。
...
在处理任何应用的授权要求时，有三个基本要素是设置的一部分：

*   需要保护/授权的资源
*   属于这些角色的角色和用户的列表
*   资源和角色之间的映射，定义谁可以访问什么

从应用的 Angular 来看，资源是页面，有时是需要限制为特定角色的页面部分。如果用户处于特定角色，根据角色资源映射，他们可以访问某些页面；否则，他们将被拒绝访问。

虽然 Angular 应用中的授权可以通过多种方式实现，但我们将概述一种通用实现，该实现可以进一步定制，以满足您未来的需求。



# 添加授权支持

要启用授权，我们需要做的第一件事是在整个应用中公开登录的用户数据，包括他/她的角色。
...
这就结束了身份验证和授权的实现。参考实现演练应该帮助我们在应用中构建身份验证和授权。有了这一基本理解，可以调整任何设置以处理其他自定义身份验证/授权场景。
...
*   我是否应该将旧 AngularJS 应用迁移到最新的 Angular 版本？
*   迁移应该在什么时候发生？
*   迁移是一次性的还是可以以增量方式完成？
*   所涉及的工作是什么？
*   我今天能做些有助于未来迁移的事情吗？
*   我今天要启动一个新的 AngularJS 应用。我应该怎么做才能使迁移在将来无缝进行？

每一个这样的问题都需要解决，以确保过渡尽可能顺利。没有人喜欢游戏中的惊喜！在接下来的部分中，我们将尝试回答一些这样的问题。作为学习的一部分，我们还将引导您完成将 AngularJS 版本的*Trainer*应用（为本书的第一个版本开发）迁移到 AngularJS 的过程。这将帮助每个人在何时和如何迁移到 Angular 上做出明智的决定。
...
在决定转向 Angular 时应注意的事项：

*   **更好的行为封装**：诚然，当我们开始学习 AngularJS 时，AngularJS*范围*似乎是上帝派来的，但我们现在已经意识到管理范围的层次性是多么困难。Angular 中基于组件的开发在应用状态方面提供了更好的封装。组件管理自己的状态、接受输入并引发事件；明确划分责任，易于推理！
*   **应用代码**中的框架较少：您不需要*范围等特殊对象。*DI 使用注释（打字）。你不会摆好手表的。总而言之，在读取组件代码时，您不会在其中找到框架级构造。
*   **需要探索的更小的框架 API**：AngularJS 有许多必须注意的指令。使用 Angular 模板语法，与浏览器事件相关的指令将消失。这减少了需要注意的指令的数量。
*   **性能**：Angular 比上一代更快。本书的完整部分致力于理解 Angular 是如何成为高性能框架的。
*   **移动友好**：Angular 试图利用服务器端渲染和 web workers 等技术优化用户的移动体验。移动设备上的 Angular 应用比其前身的性能更好。
*   **跨平台**：在大多数设备和跨平台上运行的 Angular 目标。您可以使用 Angular 为 web 和移动应用构建应用。正如我们前面所了解的，渲染层的分离在可以利用 Angular 的方面打开了大量的可能性。

在真正意义上，Angular 取代了它的前身，在一个完美的世界中，每个人都应该致力于更好的框架/技术。但是迁移从来都不是一件容易的事，特别是在这种情况下，这两个框架有很大不同。

我们的建议是，即使您现在不打算迁移到 AngularJS，也要以允许将来轻松迁移到 AngularJS 的方式构建您的 AngularJS 应用。

下一节将讨论 AngularJS 今天要遵循的原则和实践，以便在将来轻松迁移。



# 现在开发 AngularJS 应用以方便迁移

新的 AngularJS 是一种范式转换，我们在 AngularJS 中开发组件的方式与 AngularJS 完全不同。为了便于迁移，Angular 也应该采用基于组件的开发。如果我们在构建 AngularJS 应用时遵循一些准则/原则，这是可以实现的。接下来的几节将详细介绍这些指南。
...
AngularJS 作用域的最大问题是其层次性。从子作用域访问父作用域给了我们极大的灵活性，但这是有代价的。这会在不知不觉中产生不必要的依赖关系，使应用很难调试，当然也很难迁移。相反，在 Angular 中，视图绑定到其组件实现，不能隐式访问其边界之外的数据。因此，如果您计划迁移到 Angular，*不惜一切代价避免作用域*。
...
具有自己的模板和隔离作用域的 AngularJS 元素指令可以很好地表现为 Angular 组件，并且仅依赖于其视图绑定的内部状态。我们只是不需要`ng-controller`。
...
为了重申前面强调的内容，这些步骤的目标不仅仅是简单的 Angular 迁移，还包括改进 AngularJS 代码。基于组件的 UI 开发是一种比我们使用 AngularJS 更好的范例。
...
对于一个处于维护模式的应用，其中大部分开发活动都围绕着 bug 修复和一些增强，谨慎的做法是坚持使用 AngularJS。记住那句老话*如果它没有坏，就不要修理它*。
...
在迁移过程中，最好的办法是在现有应用中创建新的 SPA。例如，我们可以完全使用 Angular 构建应用的管理区域，使用单独的主机页，但如果我们稍微重构代码，仍然可以共享样式表、图像甚至 AngularJS 服务的公共基础结构。正如我们稍后将了解到的，将服务迁移到 Angular 是最简单的。

将一个应用拆分为多个较小的应用会引入整页刷新，但这是一种更干净的迁移方法。
...
欢迎来到 Angular 迁移的勇敢大世界！一个成功的迁移策略包括确保我们事先做好基础工作，避免任何后期意外。

作为准备工作，第一步是从第三方库依赖关系的 Angular 分析应用。



# 识别第三方依赖关系

AngularJS 应用使用的任何第三方库也需要迁移策略。这些库可以是基于 jQuery 的库，也可以是 AngularJS 库。
...
在接下来的部分中，我们将把 AngularJS*私人教练*应用迁移到 AngularJS。该应用目前可在*GitHub*的[上使用 http://bit.ly/a1begit](http://bit.ly/a1begit) 。该应用是本书第一版*AngularJS by Example*的一部分，使用 JavaScript 构建。
...
迁移应用的代码可从 GitHub 网站[下载 https://github.com/chandermani/angularjsbyexample](https://github.com/chandermani/angularjsbyexample) 。由于我们分块迁移，我们创建了多个检查点，这些检查点映射到专用于迁移的**GitHub 分支**。[T0]、[T1]等分支突出了这一进展。在叙述过程中，我们将突出该分支以供参考。这些分支将包含应用在该时间点之前完成的工作。
...
3.  由于应用从**mLab**（[中托管的**MongoDB**加载其训练数据 https://mlab.com/](https://mlab.com/) ），您需要一个 mLab 帐户来托管训练相关数据。通过与他们注册来建立一个 mLab 帐户。拥有 mLab 帐户后，需要从 mLab 的管理门户检索 API 密钥。遵循 API 文档（[中提供的说明 http://bit.ly/mlab-docs](http://bit.ly/mlab-docs) 获取 API 密钥。
...
7.  安装`http-server`；它将充当我们 v1 应用的开发服务器：
...
v1*私人教练*起始页应显示。使用应用验证应用是否正常工作。现在，迁移可以开始了。
...
由于我们将创建许多新的 Angular 组件，这些组件分布在许多小文件中，因此添加直接脚本引用将非常繁琐且容易出错。Angular CLI 在这里也没有多大帮助，因为它无法管理 JavaScript 中实现的现有代码库。

我们需要一个**模块加载器**。模块加载器（*ES6 模块*和非 Angular）可以帮助我们：
...
我们打开`index.html`并删除应用脚本的所有脚本引用。应删除源代码为`src='js/*.*'`的所有脚本引用，但`angular-media-player.js`和`angular-local-storage.js`除外，因为它们是外部库。
...
前面的`SystemJS.import`调用通过加载`js/app.js`中定义的第一个应用模块（JavaScript）来加载应用。我们将很快定义这个 JavaScript 模块。
...
最后几行使用`angular.bootstrap`函数引导 AngularJS 应用。
...
将`config.js`内容替换为`ng6-checkpoint8.1`或[中可用的更新内容 http://bit.ly/ng6be-8-1-configjs](http://bit.ly/ng2be-8-1-configjs) （记住再次设置 API 密钥）。更新修复了`config`功能，使其小型化友好。该测试应用了！

确保 TypeScript 编译器在一个控制台中运行；在新的控制台窗口中运行`http-server -c-1`。

导航至`http://localhost:8080`应加载的应用起始页。
...
我们首先为我们的应用安装 Angular 相关的*npm 模块*。我们将首先用必要的包更新`package.json`文件。
...
这指示 TypeScript 在`node_modules`文件夹中查找类型定义。请记住，Angular 类型是作为 Angular 库本身的一部分绑定的，因此不需要单独的类型定义导入。

现在已经添加了特定于 Angular 的引用，我们需要修改现有的引导过程以同时加载 Angular。
...
*   引导加载 AngularJS 和 Angular 框架的应用。这是我们要做的第一件事。
*   在 AngularJS 视图中合并 Angular 组件。
*   在 Angular 视图中合并 AngularJS 组件，尽管有一些限制。
*   跨框架共享依赖关系。
...
让我们学习如何使用`UpgradeModule`引导混合 AngularJS 和 Angular 应用。



# 引导混合应用

要引导混合应用，我们必须引导应用的 Angular 和 Angular 部分。该过程涉及的顺序如下：

1.  引导 Angular 应用
2.  然后，使用`UpgradeModule`引导 AngularJS 应用

由于刚刚添加了 Angular，我们需要为 Angular 定义根应用模块。
...
现在我们将 Angular 根模块定义添加到`app.module.ts`。

将`app.module.ts`的内容替换为 Angular 模块定义。从`ng6-checkpoint8.2`复制新定义（GitHub 位置：[http://bit.ly/ng6be-8-2-app-module-ts](http://bit.ly/ng6be-8-2-app-module-ts) ）。
...
Angular 框架将调用此函数作为应用引导的一部分。此函数在内部使用`UpgradeModule`的`bootstrap`函数来引导 AngularJS 应用。此函数采用与`angular.bootstrap`函数相同的参数。

虽然我们已经为 Angular 定义了根模块，但仍然没有为 Angular 应用定义入口点。在应用文件夹中创建一个新文件`main.ts`，并添加以下代码：
...
执行此代码时，指示 Angular 框架使用[T0]引导应用。语句`import './app'`有助于加载`app.ts`中的所有脚本引用。在继续之前，请记住从`app.ts`（完整的`angular.element`块）中删除 AngularJS 应用引导代码。

刷新你的应用，确保它与以前一样工作。注意 TypeScript 编译器控制台窗口或浏览器控制台日志中的错误。

祝贺我们现在有一个混合应用启动并运行。这两个框架现在正在协同工作。
...
# 将 Angular 组件注入 AngularJS 视图

最常见的迁移模式涉及将较低级别的 AngularJS 指令/视图迁移到 Angular 组件。如果我们将 AngularJS HTML 视图结构可视化为一个指令树，那么我们从叶子开始。我们将指令/视图的一部分移植到 Angular 组件，然后将该组件嵌入 AngularJS 视图模板中。此 Angular 组件作为*元素指令*注入 AngularJS 视图。
...
这是一种自底向上的方法，用于将视图/指令移植到 Angular 组件。下图突出显示了 AngularJS 视图层次结构如何逐渐转换为 Angular 组件树：
...
首先，如果您查看`exercise-nav.component.ts`文件，组件模板与 AngularJS 中使用的`left-nav-exercises.html`类似，只是没有`ng-controller`，模板绑定是基于 Angular 的：
...
可以使用`ngUpgrade`库函数`downgradeComponent.`将`ExercisesNavComponent`转换为 AngularJS 指令正如函数名所示，我们正在将 Angular 组件降级为 AngularJS 元素指令。
...
请注意，我们没有去掉[T0]，因为 AngularJS 仍然加载[T1]作为路由转换的一部分，但内部视图是一个 Angular 组件。
...
虽然我们只迁移了一个微不足道的组件，但本练习强调了将 Angular 组件转换/降级为 AngularJS 指令并在 AngularJS 视图中使用它是多么容易。Angular 组件的整体封装使这项工作变得简单。

此降级组件甚至可以使用熟悉的 Angular 特性绑定语法从父作用域获取输入：
...
从 AngularJS 到 AngularJS 的迁移是可能的，因为这些框架可以共存，并且可能共享数据。有些接触点可以跨越边界。为了更好地了解混合应用是如何工作的，以及在这种设置中可以实现什么，我们需要了解两个框架之间的协作领域。
...
虽然这是一个 Angular 组件，但主体元素归 AngularJS 所有。这意味着所有 AngularJS 模板语法都有效：
...
*   使用 AngularJS 和 AngularJS 的模板功能：
    *   嵌入 AngularJS 视图中的 Angular 组件可以使用事件和属性绑定从父范围获取输入
    *   以类似的方式，如果将指令注入到 Angular 组件视图中，它也可以从父组件获取输入并调用父组件函数（通过其隔离范围）
...
依赖项在混合应用中的注册方式取决于 DI 在这两个框架中的工作方式。对于 AngularJS，只有一个全局喷油器，而 AngularJS 有一个分层喷油器的概念。在混合环境中，最不常见的分母是两个框架都支持的全局注入器。
...
`MyService`现在可以像其他服务一样，在 AngularJS 应用中注入。
...
在混合应用中，变化检测由 Angular 控制。如果我们习惯于在代码中调用`$scope.$apply()`，我们就不需要在混合应用中这样做。

我们已经讨论了 Angular 变化检测的工作原理。Angular 框架通过在标准触发点上内部调用`$rootScope.$apply()`来触发 AngularJS 更改检测。

既然我们了解了交战规则，就更容易理解事情是如何运作的，什么是可行的，什么是不可行的。

让我们设定一些更大/更丰满的目标，并迁移 v1 应用的开始和结束页面。
...
如果*开始*页面变成 Angular 组件，我们需要将过滤器转换成 Angular 管道，并以某种方式使`translate`指令以 Angular 工作。

我们至少有以下两种选择来处理此迁移场景：

*   创建一个新的过滤器，并使用`UpgradeModule`升级 v1`translate`指令
*   在 Angular 世界中为*Angular 平移*找到合适的替代品
...
有时，应用的各个部分可能会以自顶向下的方式进行迁移；高阶视图将转换为零部件。在这种情况下，我们不迁移 AngularJS 视图中的所有自定义指令，而是使用`UpgradeModule`中定义的一些接口和函数将它们升级为 Angular 组件。下图说明了此迁移路径：

![](img/00089.jpeg)

Angular 框架对可以升级为 Angular 组件的内容进行了一些限制。以下是《Angular 偏移指南》的摘录。

要实现 Angular 兼容，AngularJS 组件指令应配置以下属性：
...
与 Angular 组件降级相比，在这个庞大的列表中，将 AngularJS 指令升级为 AngularJS 指令是困难的。通常，如果父视图已迁移到 AngularJS，我们必须对 AngularJS 指令执行实际的代码迁移。
...
[T0]图书馆现在可以使用了。我们要做的第一件事是在应用启动后立即设置默认翻译语言。



# 使用 ngDoBootstrap 进行初始化

幸运的是，对于 AngularJS 应用，`AppModule`已经定义了一个函数`ngDoBootstrap`，框架在引导 AngularJS 应用时调用该函数，这是进行`ngx-translate`初始化的理想场所。使用以下代码段更新`app.module.ts`中的`ngDoBootstrap`函数：
...
`ng6-checkpoint8.4`与[共享的管道【定义】一起复制 http://bit.ly/ng6be-8-4-shared](http://bit.ly/ng6be-8-4-shared) 并将其本地添加到`js/shared`文件夹中。另外，请记住将共享模块（`shared.module.ts`导入到应用模块（`app.module.ts`中）。我们在这里不会详细讨论任何管道实现，正如我们在前面的章节中已经做过的那样。

启动和完成组件实现已完成。让我们将它们集成到应用中。
...
翻译仍然不起作用，因为启用翻译的顶部导航代码（`root.js`仍在使用较旧的库。我们需要完全去掉 Angular 平移（v1 库）。让两个库做同样的工作并不是我们想要的，但是删除它也不是那么简单。



# 摆脱 Angular 平移

要摆脱 Angular 平移（v1）库，我们需要：

*   从所有 AngularJS 视图中删除 AngularTranslate 的指令/过滤器引用
*   删除任何使用此库的代码

完全摆脱 v1 指令/过滤器是一项困难的任务。不，我们不能在 AngularJS 视图中使用 v2`ngx-translate`管道。此外，使用 v1 translate 指令/过滤器一次性将每个视图移植为 Angular 视图也是可行的。一定有更好的解决办法吗？还有！
...
我们现在可以走了。尝试新的实施方案；应用应使用*ngx translate*库加载翻译。
...
我们将在这里停止，并向您介绍与迁移相关的其他 GitHub 分支。所有以`ng6-checkpoint*`开头的分支都是迁移分支。尝试迁移挂起的视图，并将它们与 GitHub 分支更改进行比较。记住，Angular 中已经开发了一个应用的工作版本，因此有一个很好的参考点。查看每个分支的`README.md`文件，了解应用的哪个部分已迁移到 Angular。
...
*   **迁移非常耗时**：迁移绝对不是一项琐碎的工作。每个页面/视图都提出了我们需要克服的挑战。有些元素很容易迁移，有些则不然。如果您现在正在使用 AngularJS 进行开发，您现在能做的最好的事情就是遵循*今天开发 AngularJS 应用以便于迁移*部分的建议。
*   **先迁移第三方库**：迁移第三方库可能非常具有挑战性。原因是多方面的：
    *   这样的库可以跨页面使用
    *   它们可能无法升级到 Angular（使用`UpgradeAdapter`）
    *   当库被广泛使用时，迁移使用此类库的每个视图可能不可行

最好确定应用中的所有第三方依赖项，并在 Angular 世界中为它们找到合适的替代方案。如果可能，与新库一起开发一些**概念验证**（**POC**），以了解新库与现有实现之间的差异。

*   **可能存在重叠的库**：迁移时，可能会出现库的 AngularJS 和 Angular 版本共存的情况。尽量缩短此时间段，并尽快迁移到较新版本。
*   **与**相反，将 Angular 组件集成到 AngularJS 中更容易：迁移时，将整个视图迁移到 AngularJS。由于 AngularJS 元素所施加的限制，很难使用嵌入 AngularJS 元素指令的父 Angular 组件。有了这些限制，自底向上的迁移方法比自顶向下的迁移方法更有效。
...
在本章中，我们对围绕 Angular 开发的一些实际问题获得了一些有用的见解。在使用框架构建实际应用时，这些提示/指南非常方便。

本章首先探讨了*种子项目*的概念，以及这些项目如何让我们在短时间内启动并运行。我们研究了一些流行的种子项目，它们可以作为任何新的 Angular 应用开发的基础。

尽管身份验证和授权是一个服务器端问题，但它确实会影响客户端实现。关于身份验证/授权的部分介绍了如何在基于 cookie 和令牌的设置中处理身份验证。

我们研究了性能这一非常重要的话题，在这里您学习了优化 Angular 应用性能的方法。
