...
与`WorkoutHistoryTracker`和`WorkoutService`一样，我们也将使用 mocking 来处理角路由上的依赖关系。但在这里，我们将采取稍微不同的方法。我们将在模拟游戏中为`navigate`方法分配一名茉莉花间谍：
...
这对于我们的目的来说已经足够了，因为我们只想确保路由的`navigate`方法是以适当的路由（`finished`作为参数来调用的。茉莉花间谍将允许我们这样做，我们将在后面看到。
