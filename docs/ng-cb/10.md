...
4.  现在我们需要为`VersionControlComponent`的测试设置线束环境。为此，我们将使用 Angular CDK 中的`HarnessLoader`和`TestbedHarnessEnvironment`。更新`version-control.component.spec.ts`文件如下：
