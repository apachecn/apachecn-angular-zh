...
虽然`@angular/pwa`原理图有助于快速建立一个Angular PWA 项目，但有些情况下我们需要手动完成上述所有步骤。例如:
...
规则写在`ngsw-config json`文件中。顶级Angular服务工作器配置对象接口指示有五个主要属性可以使用。
...
通过 ng CLI 为 PWA 原理图设置Angular后，得分为 100
