...
在前一章中，我们实现了 Angular Service Worker，并看到 Angular CLI 帮助我们以最小的努力运行 PWA。基本配置是我们创建带棱角 PWA 之旅的开始。很明显，随着应用的发展，它将需要先进的技术和策略。因此，Angular Service Worker 提供了更多功能来处理各种情况。
...
在单页面应用中，路由在前端处理。前端中的所有路线最终被指向`index.html`，其中框架，特别是Angular路由器模块，将导航请求匹配到特定视图。
