## 五、在 Visual Studio 中创建一个Angular 单页应用

本章指导您使用 Visual Studio 创建一个有Angular的**单页应用**（**SPA**）的过程。
...
Gulp 是在`node.js`上运行的任务运行程序。使用 Gulp，我们可以自动执行诸如移动或复制文件、捆绑和缩小等活动。在 ASP.NET Core中，Microsoft 还将 Gulp 与 Visual Studio 集成，因为它已被 web 社区广泛接受，可以非常轻松地运行和管理复杂任务。您可以访问官方网站[了解更多信息 http://gulpjs.com/](http://gulpjs.com/)
...
7.  添加一个`system.js`配置文件`systemjs.config.js`并用以下内容更新它。这具有在运行应用时加载Angular库的映射信息：
...
我们创建了一个项目来开发`My Todo`应用，并使用 NPM package manager 管理所有客户端依赖项。我们还使用 Gulp 运行一个任务，以便将 JavaScript 库复制到`wwwroot`节点。在下一节中，让我们为应用创建所需的Angular组件。

## 增加Angular组件和模板

* * *

我们将使用 TypeScript 为应用编写Angular组件。TypeScript 文件应编译为 JavaScript，目标是 ECMAScript 5。
...
我们使用 TypeScript 编译器选项配置了项目。现在，让我们为应用添加一个Angular根组件。请按照以下步骤执行此操作：
...
在上一节中，我们创建了一个名为`AppComponent`的Angular组件。现在我们需要引导这个 Ty1 T1，这样Angular会考虑这个作为应用的根组件。我们可以通过使用`NgModule`装饰`AppModule`类并添加分配给`AppComponent`的元数据引导来引导组件。按照以下步骤创建`AppModule`：
...
在这里，platform browser dynamic 包含Angular特性，通过引导应用模块使应用在浏览器中运行。如果应用的目标不是在浏览器上运行，则可以忽略这一点。
...
我们在本章开始时创建了一个新的 ASP.NET Core空应用，并更新了`Startup`类，以便为任何请求提供静态和默认页面。然后，您学习了如何在 VisualStudio 中使用 NPM 管理客户端软件包，我们在 VisualStudio 中使用 Gulp 成功地自动化和运行任务。接下来，您学习了如何为应用添加所需的组件并引导它。之后，我们设计了一个模型类和一个服务类来处理应用的核心业务逻辑。最后，我们设计了一个模板来列出`Todo`项，还添加了一些控件，并将它们连接到`TodoService`和`AppComponent`中的某些方法。
