...
*   它支持跨平台应用开发，例如高性能应用，如 web 应用、使用 Ionic Framework 的本机应用、NativeScript、React native，以及通过使用Angular方法访问本机 OS API 来创建桌面安装的应用。
*   Angular 2 继承 Angular 1 的所有优点。它用组件替换控制器和指令。
*   Angular 2 是用 TypeScript 编写的，它还使开发人员能够使用 TypeScript 编写 Angular 2 应用。
*   Angular 2 比Angular 1 快得多。新组件路由器仅加载渲染请求的视图所需的代码。模板语法使开发人员能够使用强大的模板语法快速创建视图。
...
*   **Typescript**：这是最流行和最受欢迎的Angular传输工具之一。Angular 团队与 Typescript 团队合作，共同构建 Angular 2。
...
*   Angular动画有自己的包，这意味着您不需要将动画包发送到不需要动画的项目。
*   模板标记现在已被弃用，因为它会导致与 web 组件中使用的模板 HTML 标记混淆。因此，ng 模板被引入到Angular模板中。
...
VisualStudio 代码是用于编写Angular应用的最伟大的编辑器之一。因此，我们从安装 VisualStudio 代码开始。导航到[https://code.visualstudio.com/](https://code.visualstudio.com/) 点击**`Download Code for Windows`**。Visual Studio 代码支持 Windows、Linux 和 OS X 等平台。因此，您也可以根据需要将其下载到其他平台。
...
到目前为止，我们已经了解了如何通过从 GitHub 克隆官方 QuickStart 存储库来创建 Angular 应用。在接下来的章节中，我们将详细介绍创建Angular应用的步骤。请注意，脚本是使用 System.js 加载的。System.js 是在运行时加载模块的模块加载器。
...
Angular应用通常从设计带有Angular标记或标记的模板开始。然后，我们编写组件来处理模板。特定于应用的逻辑将添加到服务中。最后，起始组件或根组件将传递给Angular引导程序。

当我们运行应用时，Angular 负责将模板呈现给浏览器，并根据组件和指令中提供的逻辑处理用户与模板中元素的交互。

让我们从以下几点来了解每个Angular块的目标：

*   任何Angular应用都将由一组组件组成。
*   服务将被注入到组件中。
*   模板负责以 HTML 的形式呈现组件。
*   组件包含支持视图或模板的应用逻辑。
