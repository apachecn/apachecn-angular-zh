## 九、Angular 和 ASP.NET Core的新功能

本书开始时，我们讨论了 Angular、构建 hello world 应用以及 Angular 的核心架构。然后，我们深入到 Angular 的构建块中。接下来，我们讨论了 TypeScript 的基础知识以及如何在编写应用时使用 TypeScript。之后，我们在 VisualStudio 中完成了创建 Angular 单页应用的过程，并学习了 RESTful 服务以及使用 Web API 为 Angular 应用创建 RESTful 服务。之后，我们在 VisualStudio 中使用 Angular、ASP.NETMVC 和 WebAPI 完成了一个完整的逐步演练，介绍了如何创建应用。最后，我们学习了如何使用 Karma 和 Jasmine 测试Angular应用。

本章将讨论 Angular 和 ASP.NET Core的新功能。我们将讨论以下主题：
...
Angular 在构建应用时引入了生成 JavaScript 代码的主要更改。这使您能够知道在生成模板时模板中是否存在任何错误，而不是在运行时收到通知。此外，它还使应用能够更快地运行，因为代码生成在构建阶段已经完成。新的Angular视图引擎在使用-**时间**（**AoT**的**提前**-**时生成的代码更少。**
