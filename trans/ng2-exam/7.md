...
处理这些依赖关系有两种方法：将它们注入到我们的组件中，或者为它们创建一个可以在测试中使用的模拟或伪模型。如果依赖项足够简单，我们可以将它的一个实例注入到测试类中。但是，如果依赖项非常复杂，特别是如果它有自己的依赖项和/或进行远程服务器调用，那么我们应该模拟它。角度测试库为我们提供了这样做的工具。
