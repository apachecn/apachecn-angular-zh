...
*   **验证Angular应用**：这是一个常见要求。我们将研究如何在 Angular 中支持基于 cookie 和令牌的身份验证。
*   **Angular性能**：必须有一个常规的性能部分，因为我们试图详细说明是什么使Angular性能更好，以及您可以做些什么来加快应用的速度。
...
*   将标准资源添加到特定文件夹。
*   在我们开发基于Angular的 web 应用的情况下，设置构建包括：
...
*   **角式起动器**[http://bit.ly/ng-starter](http://bit.ly/ng-starter) ：这个种子回购协议为任何希望快速启动并运行 Angular 和 TypeScript 的人提供了一个Angular启动程序。它使用 Webpack（模块绑定器）来构建我们的文件并帮助制作样板文件。这是一个包含大量集成的完整构建系统。
...
编译完所有模板后，根本不需要将Angular编译器位发送到客户端。树抖动现在可以摆脱编译器，创建一个更精简的框架包。Angular CLI 支持 AoT 编译，可用于生产构建。
...
*   **应用层**：这是我们的代码所在的层。它使用渲染器层上的抽象构建与之交互。我们在*深入Angular指令*中看到的渲染器类是我们用来与渲染层交互的接口。
...
该框架负责 web worker 内部运行的应用代码与主 UI 线程内部运行的呈现程序之间的通信。从开发人员的Angular来看，没有明显的差异。

由于渲染器层在Angular方向上的分离，这再次成为可能。下图显示了在应用主线程上运行的层以及在 web worker 中运行的层：
...
Angular 的渲染抽象再次打开了许多集成途径，特别是在移动平台上。与在移动浏览器上运行应用不同，可以创建Angular渲染器，以利用设备的本机 UI 功能。
...
*ReactNative*和*NativeScript*等应用平台在为本地移动平台（iOS 和 Android）提供基于 JavaScript 的 API 方面已经做得非常出色，允许我们使用熟悉语言的单一代码库。Angular渲染器更进一步。通过Angular集成，可以在浏览器和移动设备之间共享大量代码。只在视图模板和视图相关服务（如对话框、弹出窗口等）方面有所不同。
...
AngularJS 上 Angular 的主要性能改进之一是*变化检测*在 AngularJS 中的工作方式。开箱即用的Angular变化检测速度非常快，可以进一步调整以获得更好的结果。

接下来的几节将深入讨论Angular变化检测。在大规模建造任何东西时，这是一个需要理解的重要话题。它还可以帮助我们调试一些场景，在这些场景中，更改检测似乎没有像广告中所宣传的那样工作。
...
在接下来的几节中，我们将探讨Angular变化检测基础设施如何工作，何时触发变化检测，以及我们如何影响Angular变化中的变化检测行为。



# 更改检测设置

这一切都始于为视图上渲染的每个组件设置Angular变化检测器。由于每个 Angular 应用都是组件的层次结构，因此这些变化检测器也设置在同一层次结构中。下图突出显示了*训练生成器*应用在某个时间点的**变更检测器层次结构**：
...
出于显而易见的原因，Angular变化检测也只有在出现上述任何一种情况时才起作用。
...
Angular变化检测使用相同的技术在区域内执行代码。此代码可以是*事件处理程序*，在完成前内部进行更多的同步异步调用，也可以是`setTimeout`/`setInterval`操作，可能再次需要 UI 更新。

Angular变化检测框架订阅执行区域的`onMicrotaskEmpty`可观测值，并在转弯完成时启动变化检测。下图突出显示了在单击按钮时运行类似于刚才描述的代码时发生的情况：
...
尽管进行了所有这些优化，但仍可能存在遍历完整组件树的性能不够的情况。当我们必须在视图上呈现一个大数据集，保持绑定完整时，这一点尤其正确。好消息是可以调整Angular变化检测机制。
...
让我们通过一个例子来了解不可变数据结构在Angular上下文中的帮助。
...
*观测值*是另一种数据结构，可以帮助我们优化Angular变化检测。
...
虽然我们使用了可观察的输出属性（`EventEmitter`类）来引发事件，*输入**属性*也可以获取可观察的。这种可观察的输入有助于优化Angular变化检测。
...
如果服务器堆栈支持此身份验证机制，则此身份验证机制最容易实现。它是非侵入性的，可能需要对Angular应用进行最小程度的更改。**基于 Cookie 的身份验证**包括设置浏览器 Cookie 以跟踪用户身份验证会话。以下序列图解释了典型的基于 cookie 的身份验证工作流：
...
正如我们所看到的，使用这种方法，不涉及Angular基础设施，或者只涉及最小的Angular基础设施。甚至登录页面也可以是标准的 HTML 页面，它只向登录端点发送数据以进行身份验证。如果用户登录 Angular 应用，则意味着该用户已通过身份验证。
...
在 Angular 中，启用令牌身份验证是一项很好的工作。它涉及设置登录页面/组件、保护条件、身份验证服务和跨应用共享的身份验证上下文。如果您开始这一旅程，请确保寻找Angular库/模块，使集成更容易。`ngx-auth`图书馆（[http://bit.ly/ngx-auth](http://bit.ly/ngx-auth) 可以是一个很好的起点。
...
从应用的Angular来看，资源是页面，有时是需要限制为特定角色的页面部分。如果用户处于特定角色，根据角色资源映射，他们可以访问某些页面；否则，他们将被拒绝访问。
...
每一个这样的问题都需要解决，以确保过渡尽可能顺利。没有人喜欢游戏中的惊喜！在接下来的部分中，我们将尝试回答一些这样的问题。作为学习的一部分，我们还将引导您完成将 AngularJS 版本的*Trainer*应用（为本书的第一个版本开发）迁移到 AngularJS 的过程。这将帮助每个人在何时和如何迁移到Angular上做出明智的决定。
...
*   **需要探索的更小的框架 API**：AngularJS 有许多必须注意的指令。使用Angular模板语法，与浏览器事件相关的指令将消失。这减少了需要注意的指令的数量。
*   **性能**：Angular比上一代更快。本书的完整部分致力于理解 Angular 是如何成为高性能框架的。
*   **移动友好**：Angular 试图利用服务器端渲染和 web workers 等技术优化用户的移动体验。移动设备上的Angular应用比其前身的性能更好。
*   **跨平台**：在大多数设备和跨平台上运行的Angular目标。您可以使用 Angular 为 web 和移动应用构建应用。正如我们前面所了解的，渲染层的分离在可以利用Angular的方面打开了大量的可能性。
...
具有自己的模板和隔离作用域的 AngularJS 元素指令可以很好地表现为Angular组件，并且仅依赖于其视图绑定的内部状态。我们只是不需要`ng-controller`。
...
为了重申前面强调的内容，这些步骤的目标不仅仅是简单的Angular迁移，还包括改进 AngularJS 代码。基于组件的 UI 开发是一种比我们使用 AngularJS 更好的范例。
...
欢迎来到Angular迁移的勇敢大世界！一个成功的迁移策略包括确保我们事先做好基础工作，避免任何后期意外。

作为准备工作，第一步是从第三方库依赖关系的Angular分析应用。
...
由于我们将创建许多新的Angular组件，这些组件分布在许多小文件中，因此添加直接脚本引用将非常繁琐且容易出错。Angular CLI 在这里也没有多大帮助，因为它无法管理 JavaScript 中实现的现有代码库。

我们需要一个**模块加载器**。模块加载器（*ES6 模块*和非Angular）可以帮助我们：
...
我们首先为我们的应用安装Angular相关的*npm 模块*。我们将首先用必要的包更新`package.json`文件。
...
这指示 TypeScript 在`node_modules`文件夹中查找类型定义。请记住，Angular类型是作为Angular库本身的一部分绑定的，因此不需要单独的类型定义导入。

现在已经添加了特定于Angular的引用，我们需要修改现有的引导过程以同时加载Angular。
...
*   在 AngularJS 视图中合并Angular组件。
*   在Angular视图中合并 AngularJS 组件，尽管有一些限制。
*   跨框架共享依赖关系。
...
要引导混合应用，我们必须引导应用的Angular和Angular部分。该过程涉及的顺序如下：
...
现在我们将Angular根模块定义添加到`app.module.ts`。

将`app.module.ts`的内容替换为Angular模块定义。从`ng6-checkpoint8.2`复制新定义（GitHub 位置：[http://bit.ly/ng6be-8-2-app-module-ts](http://bit.ly/ng6be-8-2-app-module-ts) ）。
...
# 将Angular组件注入 AngularJS 视图

最常见的迁移模式涉及将较低级别的 AngularJS 指令/视图迁移到Angular组件。如果我们将 AngularJS HTML 视图结构可视化为一个指令树，那么我们从叶子开始。我们将指令/视图的一部分移植到Angular组件，然后将该组件嵌入 AngularJS 视图模板中。此Angular组件作为*元素指令*注入 AngularJS 视图。
...
这是一种自底向上的方法，用于将视图/指令移植到Angular组件。下图突出显示了 AngularJS 视图层次结构如何逐渐转换为Angular组件树：
...
首先，如果您查看`exercise-nav.component.ts`文件，组件模板与 AngularJS 中使用的`left-nav-exercises.html`类似，只是没有`ng-controller`，模板绑定是基于Angular的：
...
`WorkoutService`和`WorkoutBuilderService`是注入到Angular组件中的 AngularJS 服务。
...
可以使用`ngUpgrade`库函数`downgradeComponent.`将`ExercisesNavComponent`转换为 AngularJS 指令正如函数名所示，我们正在将Angular组件降级为 AngularJS 元素指令。
...
请注意，我们没有去掉[T0]，因为 AngularJS 仍然加载[T1]作为路由转换的一部分，但内部视图是一个Angular组件。
...
虽然我们只迁移了一个微不足道的组件，但本练习强调了将Angular组件转换/降级为 AngularJS 指令并在 AngularJS 视图中使用它是多么容易。Angular组件的整体封装使这项工作变得简单。

此降级组件甚至可以使用熟悉的Angular特性绑定语法从父作用域获取输入：
...
虽然这是一个Angular组件，但主体元素归 AngularJS 所有。这意味着所有 AngularJS 模板语法都有效：
...
*   使用 AngularJS 和 AngularJS 的模板功能：
    *   嵌入 AngularJS 视图中的Angular组件可以使用事件和属性绑定从父范围获取输入
    *   以类似的方式，如果将指令注入到Angular组件视图中，它也可以从父组件获取输入并调用父组件函数（通过其隔离范围）
...
在混合应用中，变化检测由Angular控制。如果我们习惯于在代码中调用`$scope.$apply()`，我们就不需要在混合应用中这样做。

我们已经讨论了Angular变化检测的工作原理。Angular 框架通过在标准触发点上内部调用`$rootScope.$apply()`来触发 AngularJS 更改检测。
...
如果*开始*页面变成Angular组件，我们需要将过滤器转换成Angular管道，并以某种方式使`translate`指令以Angular工作。

我们至少有以下两种选择来处理此迁移场景：

*   创建一个新的过滤器，并使用`UpgradeModule`升级 v1`translate`指令
*   在Angular世界中为*Angular平移*找到合适的替代品
...
Angular 框架对可以升级为 Angular 组件的内容进行了一些限制。以下是《Angular偏移指南》的摘录。

要实现Angular兼容，AngularJS 组件指令应配置以下属性：
...
与Angular组件降级相比，在这个庞大的列表中，将 AngularJS 指令升级为 AngularJS 指令是困难的。通常，如果父视图已迁移到 AngularJS，我们必须对 AngularJS 指令执行实际的代码迁移。
...
翻译仍然不起作用，因为启用翻译的顶部导航代码（`root.js`仍在使用较旧的库。我们需要完全去掉Angular平移（v1 库）。让两个库做同样的工作并不是我们想要的，但是删除它也不是那么简单。



# 摆脱Angular平移

要摆脱Angular平移（v1）库，我们需要：

*   从所有 AngularJS 视图中删除 AngularTranslate 的指令/过滤器引用
*   删除任何使用此库的代码

完全摆脱 v1 指令/过滤器是一项困难的任务。不，我们不能在 AngularJS 视图中使用 v2`ngx-translate`管道。此外，使用 v1 translate 指令/过滤器一次性将每个视图移植为Angular视图也是可行的。一定有更好的解决办法吗？还有！
...
*   **先迁移第三方库**：迁移第三方库可能非常具有挑战性。原因是多方面的：
    *   这样的库可以跨页面使用
    *   它们可能无法升级到Angular（使用`UpgradeAdapter`）
    *   当库被广泛使用时，迁移使用此类库的每个视图可能不可行

最好确定应用中的所有第三方依赖项，并在Angular世界中为它们找到合适的替代方案。如果可能，与新库一起开发一些**概念验证**（**POC**），以了解新库与现有实现之间的差异。

*   **可能存在重叠的库**：迁移时，可能会出现库的 AngularJS 和 Angular 版本共存的情况。尽量缩短此时间段，并尽快迁移到较新版本。
*   **与**相反，将Angular组件集成到 AngularJS 中更容易：迁移时，将整个视图迁移到 AngularJS。由于 AngularJS 元素所施加的限制，很难使用嵌入 AngularJS 元素指令的父 Angular 组件。有了这些限制，自底向上的迁移方法比自顶向下的迁移方法更有效。
...
在本章中，我们对围绕Angular开发的一些实际问题获得了一些有用的见解。在使用框架构建实际应用时，这些提示/指南非常方便。
