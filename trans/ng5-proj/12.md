...
众所周知，组件是用户界面中的一个构建块。Angular应用总是有一个应用(或根)组件。这个组件(像其他组件一样)在 HTML 中有一个标签，并且有Angular地引导到那个组件中。这个应用组件(像其他组件一样)可以包含其他(子)组件。
...
*   修改`import`以包括来自Angular核心的`Output`和`EventEmitter`类:
...
*   客户组件模板包括Angular指令`(click)`，用于监听和响应用户点击`div`。它触发一个使用事件发射器输出事件的方法。
...
该选择器可以是子类型(即`Child`Angular元素的类)或模板引用(`#name`)。
