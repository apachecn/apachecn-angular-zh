...
希望这一章会对你非常有用，你会用它来写你的Angular应用的路由。请记住，您的路由可能会变得非常复杂，可能会有多个路由器模块和路由器插座。您可以尝试将所有路由放在一个路由模块中，也可以尝试将其分散到多个模块中。一个路由模块可能更简单，但当开发人员不断地更改这个文件时，这可能会导致更多的合并冲突。请记住，您可以使用 route guards 来加强安全性，允许或阻止对各种组件的访问。
