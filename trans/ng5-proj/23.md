...
| [计]选项 | 描述 |
| :-- | :-- |
| `ViewEncapsulation.Emulated` | 模拟阴影 DOM，Angular的默认模式 |
| `ViewEncapsulation.Native` | 原生阴影 DOM |
| `ViewEncapsulation.None` | 一点影子都没有 |

### 查看封装。仿真:示例

让我们创建一个带有样式的示例组件，并将`ViewEncapsulation`指定为`Emulated`。这是Angular的默认模式。这将是高级示例-ex100:
...
我们快到终点了。最后一章是关于不同的Angular资源，可以在未来进一步提高你的Angular技能。