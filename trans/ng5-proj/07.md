...
现在，让我们来看看这个项目以及其中的内容。启动 Visual Studio Code并打开文件夹 start。表 [7-1](#Tab1) 显示了内部的内容及其结构。
...
| favicon.ico | 应用图标 |
| index.html | Angular单页应用的 HTML 页面 |
| 主页面 | 启动应用的代码(稍后将详细介绍) |
| 样式. css | 全局样式定义 |
| 测试. ts | 运行应用测试的代码 |
...
3.  Main.bundle.js 加载一些模块然后调用下面的Angular系统代码:
...
4.  AppModule 已加载——它是用于引导应用的根 Angular 模块。这是一个Angular模块，不是 JavaScript 模块——它们是不同的东西(我将在本书的后面介绍Angular模块)。如果您查看 AppModule.ts，您会看到它包含以下代码行，告诉模块使用 AppComponent 进行引导:
...
下一章非常重要:它介绍了组件，有Angular的用户界面的构建块。