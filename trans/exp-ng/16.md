...
# 十六、使用 Jasmine 和Protractor框架测试 Angular 应用
...
在本章中，您将学习使用 Jasmine 和Protractor测试Angular应用的所有内容。在本章中，我们将讨论以下内容：
...
在开始测试Angular应用之前，我们必须快速复习并理解一些测试中最常用的术语：
...
现在我们知道了前面的术语，让我们了解一下测试Angular应用。
...
此时，我们已经启动并运行了应用。现在是开始测试Angular应用的时候了。
...
在前面的示例中，我们看到了编写测试脚本和测试Angular组件的基本示例。

在本节中，我们将探讨如何编写测试Angular组件的最佳实践。我们将使用上一节中创建的组件--`TestAppComponent`，并通过添加变量和方法进一步扩展测试套件。
...
伟大的您学会了编写测试脚本来测试Angular组件，包括变量和方法。

您学习了使用 Jasmine 框架的一些内置方法，例如`beforeEach`、`expect`、`toBeTruthy`和`toBe`。

在下一节中，我们将继续学习高级技术，并编写更多测试脚本，以更详细地测试Angular组件。
...
在本节中，我们将深入研究并学习测试Angular组件的一些更重要和更高级的方面。
...
`TestBed`是 Angular 提供的最重要的测试工具。它创建了一个Angular测试模块--`@NgModule`类，我们可以将其用于测试目的。
...
现在，使用前面的`beforeEach`方法，其中包含组件实例，我们将创建用于测试Angular组件的测试脚本。
...
在接下来的部分中，我们将继续了解更多关于测试Angular服务的 Jasmine 框架的信息。
...
在本节中，我们将学习如何测试Angular服务。

在大多数Angular应用中，编写服务是一个重要的核心方面，因为它执行与后端服务的交互；在组件之间创建和共享数据，并易于长期维护。因此，同样重要的是确保我们正在彻底测试我们的Angular服务。
...
正如我们为测试Angular组件定义了`beforeEach`方法一样，我们为服务定义了`beforeEach`方法。在提供者数组配置中，我们正在注册`XHRBackend`类。
...
我们讨论了测试Angular组件：测试变量和方法。我们还讨论了如何在每个测试脚本之前编写要执行的`beforeEach`方法，以及如何创建组件实例并访问其属性。我们还介绍了使用 Jasmine 框架测试Angular服务，以及测试Angular服务及其属性：变量和方法。

对于测试 Angular services，您学习了如何创建一个`beforeEach`方法，该方法将在每个测试脚本之前执行，并在每个测试脚本之前创建一次提供者和依赖项。

您学习了通过模拟服务来测试后端服务。这在独立开发Angular服务和组件时非常有用。

在下一节中，您将了解如何使用Protractor框架来测试端到端测试。
...
在前面的部分中，您学习了使用 Jasmine 进行单元测试。在本节中，您将了解如何使用Protractor框架对Angular应用进行端到端测试。

这是官方网站解释Protractor的方式：

Protractor is an end-to-end test framework for Angular and AngularJS applications. Protractor runs tests against your application running in a real browser, interacting with it as a user would.

Protractor框架打包在 Angular CLI 工具中，我们可以在主项目目录中找到创建的`e2e`文件夹：
...
Protractor是围绕 SeleniumWebDriver 的包装器，它提供了许多内置类和方法，我们可以使用这些类和方法编写端到端测试。

Protractor API 主要围绕`Browser`、`Element`、`Locators`和`ExpectedConditions`公开了各种类别和方法。

Protractor支持 Chrome、Firefox、Safari 和 IE 的两个最新主要版本，这意味着我们可以编写测试脚本并在任何/所有主流浏览器上运行它们。

为了编写端到端测试，我们需要以页面中的元素为目标，读取它们的属性，更新属性，调用附加到元素的方法，或者发送和验证数据。

我们将讨论Protractor框架中可用的各种类和方法，使用这些类和方法，我们可以通过编写端到端测试来自动化应用功能。

让我们了解可用的方法和类，我们可以使用Protractor框架使用这些方法和类。
...
在本节中，您将学习如何使用Protractor与页面中的 DOM 元素交互。
...
*   `by.binding`：使用此选项选择与单向或双向Angular绑定相关的元素：
...
我们学习了一些通过使用元素的属性和信息来定位元素的方法。有关可用方法的完整列表，请参阅 GitHub 上Protractor的官方文档。
...
在本节中，您将了解Protractor API 中可用的各种内置类和方法，我们可以使用它们编写测试脚本。
...
有关属性和方法的完整列表，请参阅 GitHub 上Protractor的官方文档。

在本节中，您了解了一些可用于编写测试脚本和自动化页面中的应用工作流的可用方法。

在下面的章节中，我们将通过示例学习使用一些内置方法。在下一节中，我们将开始使用Protractor编写测试脚本。
...
在本节中，让我们开始使用Protractor编写测试脚本。我们将使用我们在前面的章节和元素定位中看到的方法来编写测试脚本。

Protractor框架测试套件的一般语法如下：
...
为茉莉花和Protractor编写的测试套件看起来很相似。主要区别在于我们使用了`element`和`browser`模块，通过它们我们可以针对页面中的任何特定 DOM 元素。
...
# 使用Protractor编写 E2E 测试



在上一节中，您学习了使用Protractor编写第一个测试脚本。在本节中，我们将扩展我们的示例并添加更多内容。
...
# 使用Protractor编写 E2E 测试-高级



到目前为止，在前面的部分中，我们已经介绍了使用Protractor框架安装、使用和编写测试脚本。我们已经学习并实现了Protractor API 公开的内置方法和类。
...
在本节中，您学习了使用Protractor框架编写测试脚本。我们探索了框架中所有内置的可用方法，以便在编写脚本时使用。

我们注意到，编写的测试脚本与 Jasmine 测试脚本类似。我们还了解了如何使用各种方法，例如`by.css`、`by.binding`和`by.id`针对特定元素或元素集合。

我们讨论了使用Protractor框架的事件处理和绑定。
...
测试是应用开发中最关键和最重要的方面之一。在本章中，您学习了如何使用 Angular CLI、Jasmine 和Protractor框架。使用 Jasmine 和Protractor进行自动化测试可以帮助您节省时间和精力。

您学习了如何为 Angular 组件和服务编写单元测试脚本，以及如何为工作流自动化测试编写 E2E 测试用例。您详细研究了 Jasmine 框架和函数内置的Protractor框架方法和变量。

我们深入研究了定位特定元素以及检索元素集合以读取、更新和编辑属性和值。继续使用这些优秀的测试框架自动化您的应用。

在下一章中，您将学习 Angular 中的设计模式。Typescript 是一种面向对象的编程语言，因此，我们可以利用数十年来关于面向对象体系结构的知识。您还将探索一些最有用的面向对象设计模式，并学习如何从Angular应用它们。