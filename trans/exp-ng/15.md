...
最后，`glyphicons`是一种字体，包含使用Bootstrap时可能需要的所有图标。
...
在我看来，你应该使用哪一种是个人的选择，我可以预见未来几个月将发生 C#与 Java 或 PC 与 Mac 之间的激烈战争。一方面，如果您已经精通Bootstrap并在任何地方使用它，那么您也可以在这里使用它。另一方面，如果Bootstrap不是你的技能集的一部分，你可以利用这个机会来学习它，然后选择你喜欢的。

第三个选择是，如果你已经选择了材质设计（由谷歌提供的Angular设计）方法，那么你可以完全跳过这一章。我不会介意的，我保证。本章涵盖的主题包括：
...
无需更多的麻烦，让我们开始安装 Angular 的Bootstrap程序。
...
首先，如果用户不拥有资源的缓存副本，那么我们将遭受与标准 web 应用相同的副作用，因为我们的客户将不得不等待 CDN 服务于Bootstrap框架，特别是考虑到由于 Angular CLI 部署过程，我们的应用已缩小并在单个文件中提供服务。其次，我们将无法轻松控制角组件中的Bootstrap组件。

将Bootstrap与 Angular 应用集成的更好方法是使用`ng-bootstrap`包。这个包允许我们使用 Angular 指令进行Bootstrap，并在我们的组件中管理它们。在撰写本文时，这是最全面、维护良好、集成良好的软件包，它允许我们在 Angular 中使用 Bootstrap。

为了探索 Bootstrap，我们将在[第 7 章](07.html)、*异步编程使用可观察对象*和[第 9 章](09.html)、*A**中使用的 Marvel Cinematic Universe 的 JSON API 基础上，以Angular*为高级表单。
...
就是这样：我们完成了Bootstrap的初始化。让我们学习如何使用Bootstrap的Angular指令。
...
在本章中，我们更关心的是学习如何为 Angular 使用不同的Bootstrap指令，而不是学习 Sass 混合和其他表示技巧。换句话说，网格系统的高级功能不属于本章的范围。然而，在本节中，我将快速介绍什么是网格系统，并概述如何使用它。

如果您以前使用过Bootstrap，更具体地说，使用过网格系统，您可以跳过本节，直接转到下一节，在那里我们学习如何使用 accordion 指令。
...
正如您所看到的，我们有几个 CSS 类在这里工作。首先，让我们看看容器。这是强制性的，定义了应用Bootstrap网格系统的空间。然后，我们有包含`col- divs`的行。每一行占据屏幕的全部宽度，并分为列。列的实际宽度取决于在列类声明末尾使用的数字（4、8、6 等等）。知道行被分成 12 列，并且我们使用了`col-md`类前缀，我们可以推断一行的最大大小是 720px。因此，每列的宽度为 60px。在第一行中，我们使用`-1`后缀作为我们的声明；因此，我们有 60px 宽的列（即屏幕宽度的大小除以 12）。然而，在第二行，我们使用了[T3]和[T4]后缀。
...
在本节中，我们将学习如何使用一些最常用的Bootstrap指令来构建应用。
...
让我们从研究Bootstrap手风琴所需的最小 HTML 开始，如下所示：
...
到目前为止，一切都相当简单。现在，当您从 TypeScript 组件管理它时，它就变得强大了。首先，`ngb-accordion`指令有三个不同的`@Input`属性，我们可以利用它们。第一个是`activeIds`，它是`string[]`，包含您希望打开的面板的 ID。面板 ID 由`ngb-panel-0`自动生成。面板 ID 以`ngb-panel-x`格式生成。第二个`@Input`是布尔值：`closeOthers`。此选项允许您指定一次是否只打开一个面板。最后，使用`string`类型来指定手风琴的风格类型。在Bootstrap中，识别四种类型：`success`、`info`、`warning`和`danger`。
...
我们将在本章探讨的下一个指令是`ng-alert`。在Bootstrap词汇表中，警报是以彩色`div`显示给用户的重要信息。存在四种类型的警报：`success`、`info`、`warning`和`danger`。

要创建Bootstrap警报，最低可行的 HTML 模板如下所示：
...
为了使表单更具Bootstrap性，我们可以将错误消息替换为警报。目前，错误消息的形式如下所示：
...
如果字段无效，我们会收到输入和Bootstrap警报。默认情况下（即字段为空时），Bootstrap警报处于活动状态。让我们将输入转换为以下内容：
...
还有一些其他的Bootstrap组件，我们可以用来增强我们的形式；然而，在可用性方面，太多会很快成为一种过度使用的情况。例如，将进度条集成到表单中是很棘手的。然而，我们所能做的是，为我们想要测试的每个新Bootstrap指令在我们的手风琴中添加一个面板。
...
在本章中，我们看到了一些最流行的Bootstrap组件。我们学习了如何将它们与 ng2 Bootstrap包提供的本机 Angular 指令一起使用。然而，我们并没有探究其中的每一个Bootstrap组件。您可以查看托管在[的官方文档 https://ng-bootstrap.github.io/](https://ng-bootstrap.github.io/) 。
