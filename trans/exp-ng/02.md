...
我们可以采用增量方法，在同一个应用中运行 AngularJS 和 AngularJS，并从组件开始逐个启动迁移过程。这种方法有助于迁移大型应用，使业务免受任何影响，并在一段时间内完成升级。这种方法可以通过使用Angular升级模块来实现。
...
AngularJS 允许使用[T0]指令对应用进行声明性Bootstrap。但是，Angular 不支持声明式Bootstrap。它只支持通过调用 bootstrap 函数并传递应用的根组件来显式Bootstrap应用。
...
请注意，在 AngularJS 中，Angular 模块名称`packtPub`已分配给`ng-app`指令。但是，在 Angular 中，我们根据执行环境将`AppModule`传递给Bootstrap模块。请注意，`AppModule`是`NgModule`类，它是我们刚刚根据执行环境Bootstrap的应用的根模块。
...
*   **更好的性能**：Angular 支持更快的更改检测、更快的Bootstrap时间、视图缓存、模板预编译等。
...
*   **更好的生态系统**：Angular生态系统最终会变得更好、更有趣。
...
使用`UpgradeAdapter`可以无缝完成增量升级。`UpgradeAdapter`是一项服务，可以Bootstrap和管理支持 AngularJS 和 AngularJS 代码的混合应用。`UpgradeAdapter`允许您同时运行 AngularJS 和 Angular 代码。`UpgradeAdapter`促进从一个框架到另一个框架的组件和服务之间的互操作性。依赖注入、DOM 和更改检测中的互操作性将由`UpgradeAdapter`负责。
...
# 向 AngularJS 注入Angular依赖关系



有时，为了在 AngularJS 代码中使用Angular依赖项，可能需要降低其级别。当我们需要将现有服务迁移到 AngularJS 或在 AngularJS 中创建新服务，而 AngularJS 中编写的组件依赖于这些服务时，这是必要的。
...
AngularJS 应用将通过将[T0]指令附加到[T1]元素来Bootstrap。这将不再适用于 Angular，因为Bootstrap Angular 应用的方式有所不同。
...
现在从`index.html`文件中的`<html>`元素中删除`ng-app`属性。然后我们需要将`UpgradeModule`导入`AppModule`。要以Angular方式Bootstrap AngularJS 应用，我们需要覆盖`AppModule`中的`ngDoBootstrap`函数，如下所示：
...
最后，我们需要在`main.ts`中Bootstrap`AppModule`，它被配置为`system.config.js`中应用的入口点。`main.ts`的代码片段如下：
...
作为升级组件的一部分，我们需要创建一个降级的Angular组件，以便 AngularJS 代码可以使用它。以下是降级Angular组件的代码段：
...
Angular 完全重新定义了路由器。逐模块升级路由器是一种很好的做法。Angular 有一个特殊的标记`<router-outlet>`，用于显示或加载布线视图。这应该在根组件的模板中。因此，对于您的应用，我们需要创建一个名为`AppComponent`的根组件：
...
最后，让我们更新`AppModule`以导入`AppRoutingModule`，并且我们已经到了删除`ngDoBootstrap`的阶段，因为在这个阶段，一切都是有Angular的。以下是`AppModule`更新后的代码片段：
