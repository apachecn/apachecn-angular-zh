...
在本章中，我们将学习和理解有关Angular指令和变化检测的所有内容。
...
变化检测是从Angular 1.x 到Angular 2 的最重要变化之一。
...
默认情况下，Angular更改检测将始终检查值是否已更改。在树结构中，从顶部根组件到内部组件始终执行相同的更改检测。

这是通过更改检测器对象对所有组件执行的。

使用`ngZones`，Angular应用的性能大幅提高。
...
参考我们在上一节中创建的示例，*在Angular*中实现变化检测。我们正在更新属性，默认情况下，Angular 使用带有`Default`值的`ChangeDetectionStrategy`。
...
我们使用`OnPush`来提高Angular应用的性能。我们必须明确指出，我们希望使用`ChangeDetectionStrategy`的`OnPush`值。
