...
在本章中，我们将学习如何安装用于材质设计的Angular组件，然后使用一些最流行的组件。我们还将看到材质图标。更详细地说，我们将看到：
...
接下来，我们需要选择一个主题。主题是将应用于Angular材质组件的颜色集。在主题中，有以下颜色：
...
此外，还可以使用Angular`[color]="primary" | "accent" | "warn"`属性。此外，工具栏还可以使用`<md-toolbar-row>`标记包含行：
...
不用说，我们是在指令方面被涵盖的。尽管有如此广泛的可能性，我们可以通过创建自定义主题来进一步自定义Angular的材质设计。在“Angular材质”中，通过组合多个选项板创建主题。具体而言，主题包括：
