...
功能模块的目的是有选择地向应用程序的其余部分公开文件夹的内容。这个模块的`@NgModule`装饰器只使用`providers`属性来定义`Model`和`SimpleDataSource`服务的类提供者。当您在特性模块中使用提供者时，它们被注册到根模块的注入器中，这意味着它们在整个应用程序中都是可用的，这正是示例应用程序中的数据模型所需要的。
