...
在这一章中，我描述了 Angular 为单元测试组件和指令提供的工具。一些有Angular的构建块，比如管道和服务，可以使用我在本章开始时设置的基本测试工具进行独立测试。组件(在较小程度上还有指令)与它们的宿主元素和模板内容有着复杂的交互，并且需要特殊的特性。表 [29-1](#Tab1) 将Angular单位测试放在上下文中。
