...
在本节中，在我们开始向 Angular 应用程序添加路由之前，我们将快速接触 base 并对一些基本概念进行概述。在基本概念中，我们将学习`Base Href`、`RouterLink`和`RouterLinkActive`，在使用角度路由时，我们需要在模板中实现这些。
...
当我们选择在应用程序中添加路由选项时，我们正在使用 Angular CLI 创建文件、导入所需模块并创建路由的规则集。
