...
这本书是为初学者和有经验的开发人员准备的，他们希望学习 Angular 或 web 开发。如果您是 Angular 开发人员，您将接触到设计 Angular 应用并将其部署到生产环境的整个过程。您将学习到易于理解和教导他人的Angular模式。如果你是一名自由职业者，你将学习有效的工具和技术，以安全、自信和可靠的方式交付你的 Angular 应用。如果您是一名企业开发人员，您将学习使用可伸缩体系结构编写Angular应用的模式和实践。
...
*第 6 章**表单、可观察对象和主题*教您如何使用Angular表单和 RxJS 的反应式编程。

*第 7 章**创建路由第一条业务线应用*，重点介绍路由第一架构，这是设计和开发中大型业务线应用的七步方法。

*第 8 章**设计认证和授权*深入探讨了Angular和 RESTful 应用中与认证和授权相关的模式。
...
*第 12 章*、*秘籍-主/详细信息、数据表和 NgRx*包含了介绍业务线应用通常需要的用户数据的秘籍，以及使用 NgRx 实现Angular通量模式的介绍。

*第 13 章*、*AWS 上的高可用云基础设施*超越了应用功能，转而在 AWS 上配置高可用云基础设施。

*第 14 章*、*谷歌分析和高级云操作*介绍了拥有、运营和优化云基础设施以及使用谷歌分析捕捉用户行为的细微差别。

*附录 A**调试Angular*介绍了如何使用 Chrome DevTool 处理常见Angular误差和断点调试。
