...
4.  要运行本章的Angular单元测试，请执行以下命令：

    ```ts
    npx ng test ch14 --watch=false 
    ```

5.  要运行本章的Angular e2e 测试，请执行以下命令：
...
让我们首先为您的应用设置一个Google Tag Manager 帐户：
...
11.  在私人窗口中，打开Angular 应用的新实例并导航到`/manager/home`路线。
...
通过利用 Google Tag Manager 和Google Analytics，我们能够在不更改 Angular 应用内任何代码的情况下完成页面跟踪。

**搜索引擎优化**（**SEO**是分析学的重要部分。为了更好地了解爬虫是如何感知你的Angular站点的，请使用谷歌搜索控制台仪表板，位于[https://www.google.com/webmasters/tools](https://www.google.com/webmasters/tools) ，以确定优化。另外，考虑使用Angular万能来在 AutoT8A.服务器端呈现某些动态内容，以便爬虫可以索引动态数据源，并向您的站点驱动更多的流量。
...
在可扩展的环境中，您无法计划 100%的利用率。配置新服务器需要时间。处于 100%利用率的服务器无法及时处理额外的传入请求，从用户的Angular来看，这会导致丢弃或错误的请求。因此，所讨论的服务器必须在达到 100%利用率之前发送触发器，以便不会丢弃任何请求。在本章前面，我建议在扩展之前将目标利用率提高 60-80%。确切的数字在很大程度上取决于您对软件和硬件栈的具体选择。
