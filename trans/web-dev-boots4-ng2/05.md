...
路由封装为 Angular 内部的模块，但它不是 Angular 核心的一部分，因此我们需要手动将其包含在`systemjs.config.js`文件中的引导配置中：
...
应用程序必须只有一个路由。我们应该对其进行配置，以便它知道如何将浏览器的 URL 映射到相应的`Route`并确定要显示的组件。实现这一点的主要方法是使用带有路由数组的`RouterModule.forRoot`函数来引导应用程序。
...
注意`filterProducts`方法。我们使用在应用程序引导中配置的路由，该路由在该组件中可用。因为我们决定使用查询参数，所以我调用了一个导航方法，并传递了与第二个参数对象相同的名称。我们可以传递任何信息，并将其转换为 URL 的查询字符串，如下所示：
...
我们所有的基本路由都已配置，现在我们可以访问所有路由的单独视图。这很好，但可能您对 URL 中的`#`符号后面的路径不满意。正如我提到的，现代 web 浏览器支持`pushState`技术，无需向服务器请求即可帮助更改浏览器中的位置和历史记录。`Router`使用此方法构建 URL。角度路由使用不同的`LocationStrategy`为新旧方式提供支持：
