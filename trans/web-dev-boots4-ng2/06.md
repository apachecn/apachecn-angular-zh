...
依赖注入可能是我所知道的最著名的设计模式之一，您可能已经使用过它。我认为这是最难解释的问题之一，部分原因是在大多数依赖注入的介绍中使用了无意义的例子。我试着想出更适合这个角度世界的例子。
...
我们在项目中使用的角度组件和自定义组件都是一组协作组件的一部分。他们相互依赖以完成其预期目的，他们需要知道：
...
Angular 框架在应用程序引导期间为我们创建了一个应用程序范围的注入器：
...
当我们需要根据引导发生后任何时刻更改的信息动态创建依赖值时，我们可以应用`useFactory`定义。
...
![Injector provider for product service](Image00105.jpg)
...
![The Cart menu component](Image00106.jpg)
...
![Update the Cart via Service](Image00107.jpg)
...
现在您将熟悉 Angular 非常依赖的依赖注入。正如我们所看到的，我们将角度代码拆分为可视组件和服务。它们彼此依赖，依赖项注入提供了引用透明性。依赖注入允许我们告诉 Angular 我们的可视化组件依赖于什么服务，框架将为我们解决这些问题。
