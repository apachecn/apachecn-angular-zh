...
![Figure 5.1 – The rxjs-operators-instance-methods app running on http://localhost:4200 ](image/Figure_5.1_B15150.jpg)
...
1.  First, we'll use the `map` operator to make sure that we are mapping the numbers generated from the `interval` Observable to the valid indices of our array. For this, we'll update the `app.component.ts` file.
    ...
    ![Figure 5.2 – The stream outputs a 0,1,2.. sequence using the modulus on inputStreamData.length ](image/Figure_5.2_B15150.jpg)
...
2.  Now, we'll use another `map` method to fetch an element from the array for each of the stream's outputs, as follows:
    ...
    ![Figure 5.3 – The stream outputs elements from inputStreamData using indices ](image/Figure_5.3_B15150.jpg)
...
![Figure 5.4 – Final output of the recipe ](image/Figure_5.4_B15150.jpg)
...
![Figure 5.5 – The rxjs-operators-static-methods app running on http://localhost:4200 ](image/Figure_5.5_B15150.jpg)
...
5.  Let's merge the streams now, and we'll add them to the appropriate output array using the `tap` operator, as follows:
    ...
    ![Figure 5.6 – Partitioned streams outputting data to the appropriate views ](image/Figure_5.6_B15150.jpg)
...
![Figure 5.7 – Logs on console for each output in the subscribe block from the merged streams ](image/Figure_5.7_B15150.jpg)
...
![Figure 5.8 – The rxjs-unsubscribing-streams app running on http://localhost:4200 ](image/Figure_5.8_B15150.jpg)
...
3.  To see whether we have an unhandled subscription, which is an issue, let's put a `console.log` inside the `startStream` method in the `home.component.ts` file—specifically, inside the `.subscribe` method's block, as follows:
    ...
    ![Figure 5.9 – The rxjs-unsubscribing-streams app running on http://localhost:4200 ](image/Figure_5.9_B15150.jpg)
...
![Figure 5.10 – The using-async-pipe app running on http://localhost:4200 ](image/Figure_5.10_B15150.jpg)
...
![Figure 5.11 – The using-combinelatest-operator app running on http://localhost:4200 ](image/Figure_5.11_B15150.jpg)
...
![Figure 5.12 – The using-flatmap-operator app running on http://localhost:4200 ](image/Figure_5.12_B15150.jpg)
...
2.  Go to the **Network** tab and simulate the **Slow 3G** network, as follows:
    ...
    ![Figure 5.13 – Simulatina slow 3G network in Chrome DevTools ](image/Figure_5.13_B15150.jpg)
...
![Figure 5.14 – Parallel calls loading data asynchronously ](image/Figure_5.14_B15150.jpg)
...
![Figure 5.15 – API calls executing synchronously ](image/Figure_5.15_B15150.jpg)
...
![Figure 5.16 – The using-switchmap-operator app running on http://localhost:4200 ](image/Figure_5.16_B15150.jpg)
...
![Figure 5.17 – A separate call sent for each input change ](image/Figure_5.17_B15150.jpg)
...
![Figure 5.18 – switchMap canceling prior HTTP calls ](image/Figure_5.18_B15150.jpg)
...
![Figure 5.19 – The using-debouncetime-operator app running on http://localhost.4200 ](image/Figure_5.19_B15150.jpg)
...
![Figure 5.20 – A new call being sent to the server on each keyboard input ](image/Figure_5.20_B15150.jpg)
...
![Figure 5.21 – debounceTime causing only one call to be sent to the server ](image/Figure_5.21_B15150.jpg)
