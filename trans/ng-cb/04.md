...
在这个配方中，您将使用淡入效果创建一个基本的双状态角度动画。我们将从一个新的 Angular 项目开始，其中已经内置了一些 UI，在应用程序中启用动画，然后开始创建我们的第一个动画。
...
![Figure 4.1 – ng-basic-animation app running on http://localhost:4200 ](image/Figure_4.1_B15150.jpg)
...
我们有一个应用程序，根本没有角度动画配置。因此，我们将从启用角度动画开始。然后，我们将用角度动画替换 CSS 动画。让我们继续执行以下步骤：

1.  首先，我们将从`app.module.ts`中的`@angular/platform-browser/animations`包中注入`BrowserAnimationsModule`，这样我们就可以在角度应用程序中使用动画。我们还将在中`imports`数组中导入`BrowserAnimationsModule`，如下所示：
...
*   角度动画（[https://angular.io/guide/animations](https://angular.io/guide/animations) ）
*   *用示例*（[解释角度动画 https://www.freecodecamp.org/news/angular-animations-explained-with-examples/](https://www.freecodecamp.org/news/angular-animations-explained-with-examples/)

# 使用多状态动画

在这个配方中，我们将使用包含多个状态的角度动画。这意味着我们将为一个特定项处理两个以上的状态。对于这个食谱，我们也将使用同样的 Facebook 和 Twitter 卡片示例。但我们将配置卡片的状态，以确定它们在出现在屏幕上之前、在屏幕上以及即将再次从屏幕上消失时的状态。
...
![Figure 4.2 – ng-multi-state-animations app running on http://localhost:4200 ](image/Figure_4.2_B15150.jpg)
...
因为你已经从之前的食谱中了解了角度动画，你可能会想，“好吧，这很容易。”好吧，是时候提高你在这个食谱中的动画技能了。您将使用此配方中的关键帧创建复杂的角度动画，以开始编写一些高级动画。
...
![Figure 4.3 – animations-using-keyframes app running on http://localhost:4200 ](image/Figure_4.3_B15150.jpg)
...
4.  Now we'll implement a zig-zag-ish animation for the appearance of the card instead of the slide-in animation. Let's add the following keyframe elements to the `keyframes` array to add a bumpy effect to our animation:
    ...
    ![Figure 4.4 – Card bumping to right and then the left wall ](image/Figure_4.04_B15150.jpg)

    图 4.4–卡向右侧和左侧墙壁的碰撞

5.  作为的最后一步，我们将顺时针旋转卡，然后将其返回到原始位置。为此，我们将使用`offset: 0.75`，使用`rotate`方法和一些附加角度。代码应如下所示：
...
令人惊叹的您现在知道如何使用`@angular/common`包中的`keyframes`方法以角度实现复杂动画。请参见下一节中的工作原理。

## 它是如何工作的…

对于角度复杂的动画，`keyframes`方法是定义动画在整个过程中的不同偏移量的非常好的方法。我们可以使用`styles`方法定义偏移量，该方法以`AnimationStyleMetadata`为参数。`AnimationStyleMetadata`还允许我们传递`offset`属性，该属性的值可以介于`0`和`1`之间。因此，我们可以为不同的偏移定义不同的样式来创建高级动画。
...
不无论您今天构建的是什么 web 应用程序，您很可能要实现某种列表。为了让这些列表更好，为什么不用它们实现一个优雅的动画呢？在本食谱中，您将学习如何使用交错动画以角度为列表设置动画。
...
![Figure 4.5 – animating-lists app running on http://localhost:4200 ](image/Figure_4.05_B15150.jpg)
...
2.  现在，在`constants`文件夹中创建一个名为`animations.ts`的文件，并添加以下代码，将名为`listItemAnimation`的基本列表项动画注册为角度：
...
令人惊叹的现在，您知道了如何在角度视图中为列表实现惊人的动画。请参见下一节中的工作原理。
...
*   *角度动画*[https://angular.io/guide/animations](https://angular.io/guide/animations)
*   角度动画错开文档（[https://angular.io/api/animations/stagger](https://angular.io/api/animations/stagger)

# 使用动画回调

在本食谱中，您将学习如何在动画状态发生角度变化时得到通知并采取行动。作为一个简单的例子，我们将使用相同的 bucket list 应用程序，并且每当添加项目的动画完成时，我们将重置`item-to-add`选项。
...
![Figure 4.6 – animation-callbacks app running on http://localhost:4200 ](image/Figure_4.06_B15150.jpg)
...
3.  Notice that both the `.start` and`.done` events are associated with the trigger name, `listAnimation`. If you refresh the app now, you should see the logs on the console as follows:
    ...
    ![Figure 4.7 – Logs on the console reflecting the .start and .done animation events ](image/Figure_4.07_B15150.jpg)
...
*   角度（[中的动画https://angular.io/guide/animations](https://angular.io/guide/animations)
...
![Figure 4.8 – route-animations app running on http://localhost:4200 ](image/Figure_4.08_B15150.jpg)
...
*   角度动画（[https://angular.io/guide/animations](https://angular.io/guide/animations) ）
*   角度路线过渡动画[https://angular.io/guide/route-animations](https://angular.io/guide/route-animations)

# 使用关键帧的角度复杂路线动画

在之前的配方中，您学习了如何创建基本路线动画，在本教程中，我们将升级我们的动画游戏。在本教程中，您将学习如何使用关键帧在角度中实现一些复杂的管线动画。
...
![Figure 4.9 – complex-route-animations app running on http://localhost:4200 ](image/Figure_4.09_B15150.jpg)
...
呜呜！我们现在为路线制作了一个 3D 动画，看起来非常棒。当然，这不是结束。当涉及到如何在角度方向上使用关键帧和动画时，天空是极限。

## 它是如何工作的…

因为我们想在这个配方中实现一个 3D 动画，所以我们首先确保动画宿主元素有一个`perspective`样式的值，这样我们就可以看到 3D 中的所有魔力。然后我们使用`keyframes`方法定义了我们的动画，每个偏移都有一个动画状态，这样我们就可以在这些状态下设置不同的角度和旋转，这样看起来很酷。我们所做的一件重要事情是使用`group`方法对`:enter`和`:leave`查询进行分组，在这里我们定义了动画。这确保了路线同时进入和离开视图。
...
*   角度复杂动画序列（[https://angular.io/guide/complex-animation-sequences](https://angular.io/guide/complex-animation-sequences)