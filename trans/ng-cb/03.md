...
![Figure 3.1 – The ng-di-token app running on http://localhost:4200 ](image/Figure_3.1_B15150.jpg)
...
![Figure 3.2 – The ng-optional-dependencies app running on http://localhost:4200  ](image/Figure_3.2_B15150.jpg)
...
1.  First, let's run the app and change the version in the input.
    ...
    ![Figure 3.3 – The logs are saved in localStorage for http://localhost:4200 ](image/Figure_3.3_B15150.jpg)
...
2.  Now, let's try to remove the configuration provided in the `@Injectable` decorator for `LoggerService`, which is highlighted in the following code:
    ...
    ![Figure 3.4 – An error detailing that Angular doesn't recognize LoggerService ](image/Figure_3.4_B15150.jpg)
...
3.  We can now use the `@Optional` decorator to mark the dependency as optional. Let's import it from the `@angular/core` package and use the decorator in the constructor of `VcLogsComponent` in the `vc-logs.component.ts` file, as follows:
    ...
    ![Figure 3.5 – An error detailing that this.logger is essentially null at the moment ](image/Figure_3.5_B15150.jpg)
...
![Figure 3.6 – The logs being printed on the console as a fallback to LoggerService not being provided ](image/Figure_3.6_B15150.jpg)
...
![Figure 3.7 – The ng-singleton-service app running on http://localhost:4200 ](image/Figure_3.7_B15150.jpg)
...
5.  In order to alert the developer about potential duplicate providers, use the `@SkipSelf` decorator from the `@angular/core` package in our `NotificationsService`, and throw an error to notify and modify `NotificationsService`, as follows:
    ...
    ![Figure 3.8 – An error detailing that NotificationsService can't be injected into NotificationsService ](image/Figure_3.8_B15150.jpg)
...
6.  All right, now we'll use the `@Optional()` decorator in `notifications.service.ts`, which is in the constructor for the dependency alongside the `@SkipSelf` decorator. The code should appear as follows:
    ...
    ![Figure 3.9 – An error detailing that NotificationsService is already provided in the app ](image/Figure_3.9_B15150.jpg)
...
![Figure 3.10 – The ng-singleton-service-forroot app running on http://localhost:4200 ](image/Figure_3.10_B15150.jpg)
...
![Figure 3.11 – The ng-aliased-class-providers app running on http://localhost:4200 ](image/Figure_3.11_B15150.jpg)
...
![Figure 3.12 – The ng-value-providers app running on http://localhost:4200 ](image/Figure_3.12_B15150.jpg)
