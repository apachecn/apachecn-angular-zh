...
![Figure 6.1 – ngrx-actions-reducers app running on http://localhost:4200 ](image/Figure_6.1_B15150.jpg)
...
8.  Finally, let's register this reducer in the `app.module.ts` file using the `StoreModule.forRoot()` method as follows so that we can see things working:
    ...
    ![Figure 6.2 – Logs showing initial state and @ngrx/store/init action on app start ](image/Figure_6.2_B15150.jpg)
...
![Figure 6.3 – Logs showing the actions for adding and removing items from a bucket ](image/Figure_6.3_B15150.jpg)
...
![Figure 6.4 – Using ngrx-store-devtools app running on http://localhost:4200 ](image/Figure_6.4_B15150.jpg)
...
4.  Open Chrome DevTools. There should be a new tab named **Redux**. Tap it and refresh the page. You'll see something like this:
    ...
    ![Figure 6.5 – Redux DevTools showing the initial Redux action dispatched ](image/Figure_6.5_B15150.jpg)
...
5.  To see the state of the app right now, tap the **State** button, as shown in the following screenshot, and you should see that we have `app > bucket: []` as the current state:
    ...
    ![Figure 6.6 – Viewing current state in the Redux DevTools extension ](image/Figure_6.6_B15150.jpg)
...
![Figure 6.7 – Redux DevTools showing addItemToBucket and removeItemFromBucket actions ](image/Figure_6.7_B15150.jpg)

图 6.7–显示 addItemToBucket 和 removeItemFromBucket 操作的 Redux 开发工具

如果您从状态展开 bucket数组，您将看到它反映了 bucket 的当前状态，如下面的屏幕截图所示：

![Figure 6.8 – Redux DevTools showing bucket's current state ](image/Figure_6.8_B15150.jpg)
...
![Figure 6.9 – addItemToBucket action causing the addition of an item to the bucket ](image/Figure_6.9_B15150.jpg)
...
![Figure 6.10 – removeItemFromBucket action causing the removal of an item from the bucket ](image/Figure_6.10_B15150.jpg)
...
![Figure 6.11 – Using ngrx-effects app running on http://localhost:4200 ](image/Figure_6.11_B15150.jpg)
...
5.  We'll now register our effect as the root effects for the app in the `app.module.ts` file, as follows:
    ...
    ![Figure 6.12 – @ngrx/effects/init action fired on app launch ](image/Figure_6.12_B15150.jpg)
...
6.  Now that we have the effects listening to the actions, let's dispatch the `GET_USERS` action from the `HomeComponent` class, and we should see the `GET_USERS_SUCCESS` action fired in return on the successful call fetch. Add the following code to dispatch the action from `home/home.component.ts`:
    ...
    ![Figure 6.13 – GET_USERS and GET_USERS_SUCCESS actions being dispatched ](image/Figure_6.13_B15150.jpg)
...
7.  Let's update the state in the `app.reducer.ts` file to listen to the `GET_USERS_SUCCESS` action and assign the users to the state accordingly. The code should look like this:
    ...
    ![Figure 6.14 – GET_USERS_SUCCESS action adding users to the state ](image/Figure_6.14_B15150.jpg)

    图 6.14–GET_USERS_SUCCESS 操作将用户添加到状态

    如果您现在查看应用程序的状态，您应该会看到如下内容：

    ![Figure 6.15 – App state containing users after the GET_USERS_SUCCESS action ](image/Figure_6.15_B15150.jpg)
...
![Figure 6.16 – Using ngrx-selectors app running on http://localhost:4200 ](image/Figure_6.16_B15150.jpg)
...
2.  Modify the `ngOnInit` method in the `home.component.ts` file. It should look like this:
    ...
    ![Figure 6.17 – Unable to display the current user and similar users ](image/Figure_6.17_B15150.jpg)
...
![Figure 6.18 – ngrx-component-store app running on http://localhost:4200 ](image/Figure_6.18_B15150.jpg)
...
![Figure 6.19 – ngrx-router-store app running on http://localhost:4200 ](image/Figure_6.19_B15150.jpg)

图 6.19–在上运行的 ngrx 路由器商店应用程序 http://localhost:4200
...
2.  Now, import `StoreRouterConnectingModule` and `routerReducer` from the `@ngrx/router-store` package in your `app.module.ts` file and set up the `imports`, as follows:
    ...
    ![Figure 6.20 – @ngrx/router-store actions and the router state reflected in the NgRx store ](image/Figure_6.20_B15150.jpg)

    图 6.20–@ngrx/路由器存储操作和 ngrx 存储中反映的路由器状态
