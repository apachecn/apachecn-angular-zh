...
Angular编程和反应式编程是最好的朋友，反应式处理应用程序的状态是应用程序可以做的最好的事情之一。NgRx 是一个框架，它提供了一组库作为 Angular 的被动扩展。在本章中，您将学习如何使用 NgRx 生态系统以反应方式管理应用程序的状态，并且您还将学习 NgRx 生态系统将帮助您完成的一些很酷的事情。
...
我们有一个Angular的应用程序，已经集成了`@ngrx/store`包。我们还设置了一个减速器，并在控制台上记录了一些操作，只要您添加或删除一个项目。让我们开始为 ap 配置应用商店开发工具：
...
了解 NgRx 是Angular和重复频率的组合（使用 RxJS）非常重要。通过使用 Store Devtools 软件包和 Redux Devtools 扩展，我们能够非常轻松地调试应用程序，这有助于我们发现潜在的 bug，预测状态变化，并更加透明地了解`@ngrx/store`软件包中幕后发生的事情。
...
如前所述，`@ngrx/component-store`是一个独立的软件包，可以轻松地安装在您的 Angular应用程序中，而无需使用`@ngrx/store`、`@ngrx/effects`等。它应该取代`BehaviorSubject`在Angular服务中的使用，这就是我们在这个配方中所做的。我们介绍了如何初始化一个`ComponentStore`以及如何在我们已经有了值而没有访问状态时使用`setState`方法设置初始状态，我们还学习了如何创建`updater`方法来更新状态，因为它们可以访问状态，甚至允许我们为自己的用例传递参数。
